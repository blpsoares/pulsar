...
const migrateCollections = async (
	...
	// try/catch mínimo em volta do fluxo inteiro (nada de ordem/lógica mudou
	// aqui dentro): sem isso, qualquer erro no meio do migrate — inclusive na
	// própria conexão — deixava o registro preso em "running" pra sempre, já
	// que `beginRun` roda antes de tudo isto e nada chamava `finishRun` de erro.
	try {
		 ...
		 */
		const clientDestination = await conn(
